...
int32_t fake_get_type_name(void* /*handle*/, char* buf, int32_t buf_len) {
  ...
  if (buf == nullptr || buf_len <= 0) return len;
...
class ClrObjectTest : public ::testing::Test {
 protected:
  void SetUp() override {
    SetClrObjectVtable(make_normal_vtable());
...
TEST_F(ClrObjectTest, MoveConstructorTransfersOwnership) {
  ...
  auto* handle = src.GcHandle();

  ClrObject dst{std::move(src)};
  EXPECT_EQ(dst.GcHandle(), handle);
  EXPECT_EQ(src.GcHandle(), nullptr);  // NOLINT(bugprone-use-after-move)
...
TEST_F(ClrObjectTest, MoveAssignmentTransfersOwnership) {
  ...
  EXPECT_EQ(g_free_count.load(), 1);
  EXPECT_EQ(b.GcHandle(), nullptr);  // NOLINT(bugprone-use-after-move)
}

TEST_F(ClrObjectTest, SelfMoveAssignmentIsNoOp) {
  ClrObject obj{make_fake_handle()};
  auto* handle = obj.GcHandle();
...
TEST_F(ClrObjectTest, TypeNameReturnsExpected) {
  ClrObject obj{make_fake_handle()};
  EXPECT_EQ(obj.TypeName(), "FakeType");
}
...
TEST_F(ClrObjectTest, IsCallableReturnsFalseInPhase2) {
  ClrObject obj{make_fake_handle()};
  EXPECT_FALSE(obj.IsCallable());
}

TEST_F(ClrObjectTest, ToDebugStringContainsTypeName) {
  ClrObject obj{make_fake_handle()};
  auto s = obj.ToDebugString();
...
class ClrObjectErrorTest : public ::testing::Test {
 protected:
  void SetUp() override {
    SetClrObjectVtable(make_error_vtable());
    ClearClrError();
  }
};
...
TEST_F(ClrObjectNullTest, MovedFromObjectIsNone) {
  ...
  EXPECT_TRUE(a.IsNone());  // NOLINT(bugprone-use-after-move)
}
...
TEST_F(ClrObjectTest, TrackerAllocCountIncrements) {
  int64_t before = GCHandleTracker::AllocCount();
  { ClrObject obj{make_fake_handle()}; }
  EXPECT_EQ(GCHandleTracker::AllocCount(), before + 1);
}

TEST_F(ClrObjectTest, TrackerFreeCountIncrements) {
  int64_t before = GCHandleTracker::FreeCount();
  { ClrObject obj{make_fake_handle()}; }
  EXPECT_EQ(GCHandleTracker::FreeCount(), before + 1);
}

TEST_F(ClrObjectTest, TrackerLeakCountIsZeroAfterConstruct) {
  int64_t pre_leak = GCHandleTracker::LeakCount();
  { ClrObject obj{make_fake_handle()}; }
  EXPECT_EQ(GCHandleTracker::LeakCount(), pre_leak);
}
