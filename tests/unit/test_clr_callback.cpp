...
{
    // Load from the RuntimeTest managed-assembly directory so that C#
    // [LibraryImport("atlas_engine")] finds the exact same DLL file,
    // avoiding a second DLL instance with separate global state.
...
public:
    static void SetUpTestSuite()
    {
        ...
        s_dll_has_error = *s_engine_lib->get_symbol<int32_t (*)()>("atlas_has_clr_error");
        s_dll_read_error =
            *s_engine_lib->get_symbol<int32_t (*)(char*, int32_t)>("atlas_read_clr_error");
        s_dll_clear_error = *s_engine_lib->get_symbol<void (*)()>("atlas_clear_clr_error");
        ...
        auto bootstrap_method =
            s_host.get_method_as<BootstrapFn>(test_dll(), kTestType, "RunBootstrap");
...
protected:
                                   ...
                                   std::string_view method_name) -> bool
    {
        ...
        if (!result)
        {
            ADD_FAILURE() << "bind_method(" << method_name
...
TEST_F(ClrCallbackTest, ErrorBridgeRelaysException)
{
    ...
    auto fn_result = s_host.get_method(test_dll(), kTestType, "ThrowException");
    ASSERT_TRUE(fn_result.has_value()) << fn_result.error().message();
...
TEST_F(ClrCallbackTest, GCHandleLifecycle)
{
    ...
    // Wrap in ClrObject — takes ownership of the GCHandle.
    {
        ...
        EXPECT_EQ(obj.type_name(), "String");

        auto str_result = obj.as_string();
        ASSERT_TRUE(str_result.has_value()) << str_result.error().message();
