...
#if defined(_WIN32)
#include <windows.h>
// windows.h defines SendMessage as a macro; undefine to avoid collision with Channel::SendMessage.
#undef SendMessage
#endif
...
struct ChildProcess {
  ...
  static auto launch(const std::filesystem::path& exe, const std::vector<std::wstring>& args,
    ...
    auto log_file =
        std::filesystem::temp_directory_path() / std::format("atlas_login_flow_{}.log", proc_label);
  ...
  [[nodiscard]] auto exit_code() const -> std::optional<DWORD> {
    if (pi.hProcess == nullptr) return std::nullopt;
    DWORD code = 0;
    if (!::GetExitCodeProcess(pi.hProcess, &code)) return std::nullopt;
    return code;
  }

  [[nodiscard]] auto read_output_tail(std::size_t max_lines = 40) const -> std::string {
    if (log_path.empty() || !std::filesystem::exists(log_path)) return "(no log file)";

    std::ifstream f(log_path, std::ios::in);
    if (!f.is_open()) return "(cannot open log file)";
    ...
    if (ring.empty()) return "(empty)";

    std::ostringstream oss;
    for (const auto& l : ring) oss << "  " << l << "\n";
    return oss.str();
  }
  ...
  void remove_log() const {
    if (!log_path.empty()) std::filesystem::remove(log_path);
  }

  void terminate() {
    if (pi.hProcess == nullptr || !is_running()) return;
...
struct ClientHarness {
  explicit ClientHarness(std::string name) : dispatcher(std::move(name)), network(dispatcher) {
    ...
    (void)network.InterfaceTable().RegisterTypedHandler<login::LoginResult>(
        [this](const Address&, Channel*, const login::LoginResult& msg) {
          login_result = msg;
          login_received.store(true, std::memory_order_release);
        });
    (void)network.InterfaceTable().RegisterTypedHandler<baseapp::AuthenticateResult>(
        [this](const Address&, Channel*, const baseapp::AuthenticateResult& msg) {
          auth_result = msg;
          auth_received.store(true, std::memory_order_release);
        });
    (void)network.InterfaceTable().RegisterTypedHandler<login::AllocateBaseAppResult>(
...
auto wait_for_process_registration(
  ...
  const auto deadline = std::chrono::steady_clock::now() + timeout;
  while (std::chrono::steady_clock::now() < deadline) {
    auto entries = machined_client.QuerySync(type, std::chrono::milliseconds(300));
    if (last_entries != nullptr) *last_entries = entries;
    for (const auto& entry : entries) {
      if (entry.internal_addr.Port() != expected_internal_port) continue;
...
auto format_process_entries(const std::vector<machined::ProcessInfo>& entries) -> std::string {
  ...
  for (const auto& entry : entries) {
    ...
    oss << std::format("name={} internal={} external={} pid={} load={:.3f}", entry.name,
                       entry.internal_addr.ToString(), entry.external_addr.ToString(), entry.pid,
                       entry.load);
  }
  return oss.str();
}
...
TEST(LoginFlowIntegration, ClientCanLoginAndAuthenticateThroughFullStack) {
  ...
  machined_dispatcher.SetMaxPollWait(Milliseconds(1));
  ...
  ChildProcess baseappmgr = ChildProcess::launch(
      baseappmgr_exe,
      {L"--type", L"baseappmgr", L"--name", L"baseappmgr_login_flow_test", L"--update-hertz",
       L"100", L"--internal-port", std::to_wstring(baseappmgr_port), L"--machined", machined_addr},
  ...
  registry_dispatcher.SetMaxPollWait(Milliseconds(1));
  ...
  ASSERT_TRUE(wait_for_process_registration(registry_client, registry_dispatcher,
                                            ProcessType::kDbApp, dbapp_port, std::nullopt,
                                            &dbapp_entries))
  ...
  ASSERT_TRUE(wait_for_process_registration(registry_client, registry_dispatcher,
                                            ProcessType::kBaseAppMgr, baseappmgr_port, std::nullopt,
                                            &baseappmgr_entries))
  ...
  ASSERT_TRUE(wait_for_process_registration(registry_client, registry_dispatcher,
                                            ProcessType::kBaseApp, baseapp_internal_port,
  ...
  ASSERT_TRUE(wait_for_process_registration(registry_client, registry_dispatcher,
                                            ProcessType::kLoginApp, loginapp_internal_port,
  ...
  auto bam_channel = client.network.ConnectRudp(Address("127.0.0.1", baseappmgr_port));
  ASSERT_TRUE(bam_channel.HasValue()) << bam_channel.Error().Message();
  ...
  while (std::chrono::steady_clock::now() < allocate_deadline) {
    ...
    alloc.type_id = 1;
    alloc.dbid = 42;
    ASSERT_TRUE((*bam_channel)->SendMessage(alloc).HasValue());
    if (poll_until(
            client.dispatcher,
  ...
  auto login_channel = client.network.ConnectRudp(Address("127.0.0.1", loginapp_external_port));
  ASSERT_TRUE(login_channel.HasValue()) << login_channel.Error().Message();

  login::LoginRequest login_req;
  login_req.username = "hero";
  login_req.password_hash = "pw_hash";
  ASSERT_TRUE((*login_channel)->SendMessage(login_req).HasValue());
  ...
  ASSERT_TRUE(loginapp.is_running()) << loginapp.diagnostic();
  EXPECT_EQ(client.login_result.status, login::LoginStatus::kSuccess);
  EXPECT_TRUE(client.login_result.error_message.empty());
  EXPECT_EQ(client.login_result.baseapp_addr.Port(), baseapp_external_port);

  client.reset_auth();
  auto baseapp_channel = client.network.ConnectRudp(client.login_result.baseapp_addr);
  ASSERT_TRUE(baseapp_channel.HasValue()) << baseapp_channel.Error().Message();

  baseapp::Authenticate auth;
  auth.session_key = client.login_result.session_key;
  ASSERT_TRUE((*baseapp_channel)->SendMessage(auth).HasValue());
