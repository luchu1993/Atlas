...
5. **Don't enumerate code files.** Resist the urge to close a design doc with a "涉及的模块与文件 / Files touched / 涉及文件" inventory. File trees rot the instant code moves, get out of sync the next refactor, and add nothing the reader can't get from `git grep` or following the in-prose pointers. Reference a key file inline by relative path when it anchors a specific decision; never compile a standalone catalog. Same applies to "modules changed", "code paths", and similar manifests — they're the mark of a doc trying to do the IDE's job.
