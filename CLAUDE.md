...
2. **Every comment block ≤ 2 lines, in headers and `.cc` alike.** Implementation files should read top-to-bottom as plain code; the *why* belongs in the header next to the API — but the header isn't a free-text essay either. No paragraph blocks, no section banners (`// ====`, `// ----`, `// ## Section`), no file-level narrative comments, no class-header bios, no historical narratives. When a comment is unavoidable, keep it ≤ 2 lines and place it immediately above the surprising line. If a thought needs more than two lines to explain, the design doc is the right place — link it instead. CMake / build scripts follow the same rule.
