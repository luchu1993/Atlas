...
public partial class StressAvatar : ClientEntity
{
    ...
    {
        if (IsOwner) AddLocalComponent<StressMovementBotComponent>();
    ...
    partial void OnMainWeaponChanged(Atlas.Def.StressWeapon oldValue,
                                     Atlas.Def.StressWeapon newValue)
    {
        Log.Info($"[StressAvatar:{EntityId}] OnMainWeaponChanged old.id={oldValue.Id} "
                 + $"new.id={newValue.Id}");
    }
    ...
    protected override void OnPositionUpdated(Vector3 newPos)
    {
        ...
        Log.Info($"[StressAvatar:{EntityId}] OnPositionUpdated pos={FormatVec(newPos)} "
                 + $"samples={samples} lat={lat:F3}");
    }

    public partial void EchoReply(uint seq, ulong serverTsNs, ulong clientTsNs)
    {
        Log.Info($"[StressAvatar:{EntityId}] EchoReply seq={seq} "
                 + $"serverTsNs={serverTsNs} clientTsNs={clientTsNs}");
        var m = Metrics;
    ...
    internal void SendMovementFrames(ReadOnlySpan<AtlasMovementInputFrame> frames)
    {
        SendMovementInput(frames);
    }

    internal void SendMovementCorrection(uint ackedInputSeq, uint serverTick, float distanceM,
                                         ushort correctionFlags)
    {
        SendMovementCorrectionReport(ackedInputSeq, serverTick, distanceM, correctionFlags);
    }
