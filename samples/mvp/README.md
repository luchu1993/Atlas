...
`src/csharp/Atlas.Client*/*.cs`, re-run
...
Unity and UE owner movement now send quantized input frames and predict locally
with the native `movement_sim` step; CellApp owns the final position and sends
movement acks for replay. Owners report post-replay correction tiers for server
watchers only; authority remains on CellApp. Movement acks also seed the next
owner input sequence after reconnect so a resumed client does not collide with
...
| **Left Shift** | Dash forward through a server-authoritative MovementCommand |
...
- **Avatar** (cyan capsule): 100 HP, respawns at origin 3 s after death.
  Death clears any active MovementCommand, so dash playback ends before respawn.
...
streams in everything within reach. Movement is server-authoritative:
`PlayerInputController` samples WASD/joystick into native input frames,
predicts locally, sends the latest frame plus two redundant frames, and
replays after `MovementStateAck`, then reports correction tier telemetry.
Left Shift calls `Avatar.Dash`, which writes a server-stamped
`MovementCommand` and broadcasts `MovementCommandStart` / `MovementCommandEnd`
to owners and peers. Death calls `ClearMovementCommand`, so an in-flight dash
ends through the same `MovementCommandEnd` path with a cancelled reason before
respawn. The dash command uses an explicit movement curve registered from the
cell script. Unity and UE register the same curve id locally and sample it during
owner prediction and remote overlay; the sample set is linear for now.
Projectile damage is server-authoritative:
`LaunchProjectile`
