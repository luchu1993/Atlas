...
1. ~~CLR Bootstrap 对客户端路径未就绪~~ **已修复**（`Atlas.ClrHost` 专用 dll + `ClrScriptEngine.Config.lifecycle_type/hotreload_type` 参数化 + `client_app.cc::InvokeDesktopBootstrap/LoadUserAssembly` 接线）。atlas_client 现在的 CLR 初始化链端到端 green：CoreCLR init → `Atlas.Core.Bootstrap, Atlas.ClrHost` → `DesktopBootstrap.Initialize` 填槽 + 注册 native callback table → `DesktopBootstrap.LoadUserAssembly` 触发生成器 `[ModuleInitializer]`。脚本钩子最终能点亮，剩下的是端到端 smoke 在真实集群里跑一遍。
