...
2. **HP delta 流尚未达到脚本回调层**（新）：init/enter 路径 green，但 `OnHpChanged` 在运行期不再补齐。服务端 `StressAvatar.OnTick` 确实在以 1 Hz 递减 Hp（通过 cellapp 日志确认），但 `ReplicatedReliableDeltaFromCell (2017) → 0xF003` 的转发在 `SelectAvatar` 之后的 `GiveClientTo` 并没有把 StressAvatar 的 proxy 与 client 关联起来，baseapp 端 `ResolveClientChannel` 对新 entity 返回 null。和 CLR 初始化分离的独立问题，属于 handoff 序列的服务端 bug（未修）。C2 的 init/enter 已可验证；hp 计数验证待该问题修复后再做。
