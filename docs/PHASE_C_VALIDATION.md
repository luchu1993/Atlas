...
3. **Baseline snapshot 取自 BaseApp 的属性副本**（新）：`BaseApp::EmitBaselineSnapshots` 调的 `SerializeForOwnerClient` 跑在 **base-side** 的 StressAvatar 实例上，而 `Hp` 的权威数据在 CellApp（OnTick 跑在 cell）。base-side `_hp` 永远是默认值 0，所以每 `kBaselineInterval ≈ 6s` 一次的 0xF002 到达时，client 的 `ApplyOwnerSnapshot` 把 `_hp` 写回 0 —— 下一条 delta 到达时触发 `OnHpChanged(0, N)` 而非 `OnHpChanged(prev, N)`。

   **Why:** Atlas 生成器在 base 和 cell 各自 emit `_hp` 字段，但没有 cell→base 的同步路径；base 上的 `_hp` 永远是空副本。

   **How to apply:** 未来要么 (a) 让 cell 在每次 PublishReplicationFrame 时把 owner_snapshot 一并 push 到 base（base 保持 cell 的投影），要么 (b) 把 baseline pump 从 baseapp 迁到 cellapp（有权威数据的进程发 baseline）。方案 (a) 改动最小。
