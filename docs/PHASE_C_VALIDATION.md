...
3. **Baseline pump 暂时禁用**（短期修复）：`BaseApp::EmitBaselineSnapshots` 原本每 `kBaselineInterval ≈ 6s` 从 **base-side** 实例调 `SerializeForOwnerClient` 序列化并以 0xF002 发给 client。但 BigWorld 的架构原则（`lib/entitydef/data_description.ipp:113-131` 的 `isCellData = !isBaseData` —— "data only lives on a base or a cell but not both"）在 Atlas 当前实现里没被遵守：generator 在 base 和 cell 都 emit `_hp` 字段，只有 cell 端的 OnTick 写，base 端永远是默认 0。baseline 到达时 `ApplyOwnerSnapshot` 把 client 的 `_hp` 覆写为 0，下一条 delta 触发 `OnHpChanged(old=0, new=N)` 而非 `OnHpChanged(prev, N)`。

   **当前状态**：`EmitBaselineSnapshots` 已改为 no-op。所有 stress 压测属性都走 0xF003 reliable channel，transport 层保证重传，不需要 baseline 兜底；C3-B（`reliable="false"` 丢包恢复）场景暂不可跑，需要 baseline 路径重建后再验证。

   **中期计划（BigWorld 对齐）**：generator 按 scope 分拆 field 生成 —— base 的 generated class 只 emit `base_and_client` 属性字段；cell 的 generated class 只 emit `own_client / all_clients / other_clients / cell_public_and_own` 字段。`SerializeForOwnerClient` 只在 cell 端生成。消除 "base 有陈旧副本" 这个歧义源。

   **长期计划（BigWorld 对齐）**：CellApp 周期性把 cell 属性 dict 序列化成 opaque bytes 通过新增的 `BackupCellEntity` 消息发给 base（对齐 BigWorld `backupCellEntity`，`bigworld/server/cellapp/real_entity.cpp:884-906`）。base 端存 `cellBackupData_` 不反序列化，只代管。DB 写入、offload 迁移、reviver 重建均改走 `cellBackupData_`。若将来要恢复 baseline，pump 应在 cellapp 侧发，走同一份 opaque bytes。
