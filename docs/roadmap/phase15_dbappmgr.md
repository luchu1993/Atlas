...
XML duplicate guard、duplicate retry、BaseApp WriteToDB death retry 和
BaseApp login checkout death retry 已接入；LoginApp auth retry、BaseApp
logoff request retry 与 P15.2 HA 未完成。
...
| P15.1-D4 | DBApp 端 request version 校验 + idempotency cache + request retry on death | version 校验、completed ack idempotency cache、BaseApp WriteToDB death retry 与 BaseApp login checkout death retry 已接入；LoginApp auth 与 BaseApp logoff death retry 未完成 |
