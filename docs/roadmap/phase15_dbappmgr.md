...
XML duplicate guard、duplicate retry 和 BaseApp WriteToDB death retry 已接入；
checkout/auth/logoff request retry 与 P15.2 HA 未完成。
...
| P15.1-D4 | DBApp 端 request version 校验 + idempotency cache + request retry on death | version 校验、completed ack idempotency cache 与 BaseApp WriteToDB death retry 已接入；checkout/auth/logoff death retry 未完成 |
