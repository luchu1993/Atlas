...
> **必须在 Phase 10 Step 10.1 启动前全部合入。**
>
...
| PR | 名称 | 规模 | 主要改动 | Phase 10 依赖点 |
|----|------|------|---------|---------------|
| **PR-A** | BaseApp `ClientCellRpc` struct + handler 骨架 | 小 | `baseapp_messages.h` 新增 struct；`baseapp.{h,cc}` 新增 handler；客户端 SDK 真实发送 | Step 10.8 / 10.9 / 10.10 |
| **PR-B** | DEF_GENERATOR Step 5 C++ 部分 | 中 | `.def` 解析器 + `RpcDescriptor.direction/exposed` 字段；`DefGenerator` 同步写 direction metadata | Step 10.8 / 10.9 |
| **PR-C** | Script Phase 4 — C# 序号 + 受众过滤 delta | 中-大 | `DeltaSyncEmitter.cs` 新增 `ReplicationFrameHandle` 产出（owner/other snapshot + delta + event_seq + volatile_seq） | Step 10.5b / 10.7 |
| **PR-D** | BaseApp 有序可靠 delta 通道（路径分离策略） | 小-中 | 文档 + 单测，明确属性 delta 禁止经 `DeltaForwarder` | Step 10.5b / 10.9 |

**并行性**：PR-A / PR-B / PR-C / PR-D 互无依赖，可完全并行。
...
## 2. 当前代码基线核查（2026-04-18）

前置 PR 的必要性基于对以下代码事实的核查。实现前请再次验证：

| 检查项 | 当前状态 | 证据 |
|---|---|---|
| `ClientCellRpc` (2023) 存在？ | ❌ 仅枚举值 | `src/lib/network/message_ids.h:98` |
| `ClientBaseRpc` (2022) 存在？ | ✓ struct + handler 齐全 | `src/server/baseapp/baseapp_messages.h:479-514`，`baseapp.cc:2122` |
| `ReplicatedDeltaFromCell` (2015) Reliable？ | ❌ Unreliable | `baseapp_messages.h:381` |
| `DeltaForwarder` 是 latest-wins？ | ✓ 同实体覆盖 | `delta_forwarder.h:25-28` 注释 + `Enqueue()` 实现 |
| `DeltaForwarder` 客户端消息 ID？ | ✓ `0xF001` | `delta_forwarder.h:51` `kClientDeltaMessageId` |
| `.def` 解析器存在？ | ❌ 仅 `FromJsonFile` | `src/lib/entitydef/entity_def_registry.h` |
| `RpcDescriptor.direction / exposed` 字段？ | ❌ 未定义 | 同上 |
| `FindRpc()` / `GetExposedScope()` 接口？ | ✓ 声明存在 | `entity_def_registry.h:43,49` |
| C# `DeltaSyncEmitter` 产出？ | ✓ owner/other snapshot + 非分组 delta | `src/csharp/Atlas.Generators.Def/Emitters/DeltaSyncEmitter.cs` |
| C# `event_seq / volatile_seq`？ | ❌ 不存在 | — |
| C# `PropertyScope` 值数量？ | 8 | `DefModel.cs:12-22` |
| `IsOwnerVisible` audience？ | `{OwnClient, AllClients, CellPublicAndOwn, BaseAndClient}` | `DeltaSyncEmitter.cs:126-136` |
| `IsOtherVisible` audience？ | `{AllClients, OtherClients}` | `DeltaSyncEmitter.cs:141-149` |
| `client_native_provider::SendCellRpc()`？ | 方法存在，实现需确认 | `client_native_provider.h:34` |
| `EntityApp / ScriptApp / ServerApp` tick 钩子？ | ✓ 全部就位 | `entity_app.h:37`，`server_app.h:89/92/95`，`script_app.h:55/65` |
| `BaseEntity.cell_entity_id_ / cell_addr_`？ | ✓ 存在 | `base_entity.h:63-64` |
...
- [`../DEF_GENERATOR_DESIGN.md`](../DEF_GENERATOR_DESIGN.md) Step 5 — PR-B 的上游设计源
- [`../BIGWORLD_RPC_REFERENCE.md`](../BIGWORLD_RPC_REFERENCE.md) Section 3.1 / 3.3 — PR-A 的 RPC 语义参考
- [`phase9_baseappmgr.md`](./phase9_baseappmgr.md) — Phase 9 已落地（BaseApp + Proxy 基础设施）
