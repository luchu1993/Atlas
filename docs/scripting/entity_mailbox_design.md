...
| Emitter | 产出 |
|---|---|
| `MailboxEmitter` | `{Entity}ClientMailbox` / `{Entity}CellMailbox` / `{Entity}BaseMailbox` `readonly struct`;以及实体上的 `Client` / `AllClients` / `OtherClients` / `Cell` / `Base` 属性。`Atlas.Core.RpcTarget` 提供 `Owner = 0 / Others = 1 / All = 2` |
| `RpcStubEmitter` | Mailbox 方法体:用 `SpanWriter` 序列化参数后调用 `SendClientRpc / SendCellRpc / SendBaseRpc` |
| `DispatcherEmitter` | 接收端 `RpcDispatcher`,按 `(slotIdx, typeIndex, direction, methodIndex)` 反序列化参数,调用实体的 `OnXxx` 处理器 |
| `RpcIdEmitter` + `RpcIdEncoder` | RPC ID 常量表,packed 格式 `reply:1 | slot:7 | direction:2 | typeIndex:14 | methodIndex:8`(参见 `entity_type_registration.md` §3) |
...
entity.Client.ShowDamage(100f, pos)
   └─> new AvatarClientMailbox(entity, RpcTarget.Owner)
         └─> var writer = new SpanWriter(24);
             writer.WriteFloat(100f);
             writer.WriteVector3(pos);
             _entity.InternalSendClientRpc(RpcIds.Avatar_ShowDamage, _target, writer.WrittenSpan);
               └─> ServerEntity.SendClientRpc(rpcId, target, payload)
                    └─> NativeApi.SendClientRpc(EntityId, rpcId, target, payload)
                         └─> [LibraryImport("atlas_engine", EntryPoint = "AtlasSendClientRpc")]
                              └─> INativeApiProvider.SendClientRpc(...)
```

`ServerEntity.SendClientRpc(int rpcId, RpcTarget target, ReadOnlySpan<byte> payload)`
显式携带目标；Mailbox struct 只保存 `RpcTarget` 并把它传到 NativeApi。
...
3. **RPC ID 稳定性**。`.def` 中方法顺序变化会让 ID 漂移;
   ...
   客户端登录携带 32-byte `EntityDefDigest`,BaseApp 用
   `EntityDefRegistry::Digest()` 比对并拒绝不一致的客户端构建;
   `PersistentPropertiesDigest()` 目前只是本地持久字段摘要 helper,尚未接入
   BaseApp ↔ DBApp 握手。
