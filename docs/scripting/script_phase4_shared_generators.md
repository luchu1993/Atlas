...
└── Rpc/
    ├── RpcAttributes.cs           # [ClientRpc], [CellRpc], [BaseRpc]
...
### `src/csharp/Atlas.Shared/Rpc/RpcAttributes.cs`
...
public partial class PlayerEntity : ServerEntity
{
    ...
    [CellRpc]  // .def: exposed: OwnClient — 客户端可调用
...
// Direction: ClientRpc=0x00, CellRpc=0x02, BaseRpc=0x03 (0x01 reserved)
internal static class RpcIds
{
    // ClientRpc (Direction=0x00)
    public const int PlayerEntity_ShowDamageNumber = 0x0001_0001;

    // CellRpc (Direction=0x02, includes exposed methods)
    public const int PlayerEntity_RequestMove      = 0x0001_0201;
}
```

### 工作点

- [ ] 扫描 `[ClientRpc]` / `[CellRpc]` / `[BaseRpc]` 标记的 `partial` 方法
- [ ] 为每个方法生成序列化存根（发送端）
...
| Rpc | `RpcGeneratorTests.cs` | ClientRpc/CellRpc/BaseRpc; 多参数; Reliable 标记; void 检查 |
