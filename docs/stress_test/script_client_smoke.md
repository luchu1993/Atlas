...
- `atlas_client.exe` 已构建（默认路径 `bin/debug/atlas_client.exe`）
- `Atlas.ClientSample.dll` 已部署到 `bin/<build>/Atlas.ClientSample.dll`；
  runtimeconfig 由脚本从 sample 项目输出自动转发
- `entity_defs/StressAvatar.def` 的 `hp` 属性为 `reliable="true"`（默认值）

脚本客户端的 owner `StressAvatar` 会挂载 `StressMovementBotComponent`：
...
`run_world_stress` 透传 `--login-rate-limit-per-ip` /
`--login-rate-limit-trusted-cidr` 给 LoginApp —— 所有本地 client 共享
`127.0.0.1`，默认 5/60s 的 per-IP 限额撑不起 50 个 client 并发登录：
...
[script-clients] per-child event summary:
  username                   init  enter  dst    hp  ...   pos    mIn   mAck   mRpt   mc1   mc2  snap   gaps  unprs
  script_user_50               53     51    0   937  ...    84   ~300   >90    >90    >=0   >=0     0      0    ~16
  script_user_51               53     51    0   937  ...    83   ~300   >90    >90    >=0   >=0     0      0    ~16
```
...
| `enter` | 51-52 | 非 self 的 peer enter 才触发（self 的 StressAvatar 走 EntityTransferred → CreateEntity 路径，未经 kEntityEnter） |
| `hp` | 800-1000 | 52 avatars × 30 s × 1 Hz ≈ 1560 个全量事件；AoI 进出、优先级回退、带宽预算会裁剪一部分；稳态稀疏约 60% 落地 |
| `pos` | 50-200 | 每个 bare 10 Hz × 30 s = 300；script 看到的只有 AoI 半径内的 peer |
| `mIn` / `mAck` / `mRpt` | `mIn` 约 10 Hz，`mAck == mRpt > 0` | owner 本地预测、ACK replay 和 correction report 发送路径都必须有事件 |
| `gaps` | **0** | 无丢包时 reliable delta 的 event_seq 必须连续；非零 = reliable channel 或 replication 路径有回归 |
| `unprs` | ≈ 16 | init 阶段的 ClR / 网络日志行，非脚本事件 |

`world_stress --script-verify` 要求每个子进程至少观察到 1 次 `OnInit`、
1 次 `mIn`、1 次 `mAck` 和 1 次 `mRpt`，否则返回非 0 exit code。通过
`run_world_stress --script-verify` 运行时，还要求 BaseApp watcher 汇总的
`in` / `fwd` / `ack` / `rpt` 和 CellApp watcher 汇总的 `in` / `frames` /
`sim` / `hist` / `ack` 都大于 0。BaseApp `rate` / `invalid` / `seqgap` /
`ackstale` / `rptdrop` 必须为 0；CellApp `rate` / `invalid` / `seqgap` /
`overflow` 必须为 0。

`run_world_stress` 在 `--script-clients > 0` 的运行结束后会打印
`Movement watcher summary`。BaseApp 行能看到输入包、转发、drop、ack 和
correction tier；CellApp 行能看到输入帧入队、drop、队列深度、模拟帧、
ack 和 step p95。单元格为 `?` 表示该 watcher 查询失败。
...
```
[script-clients] per-child event summary:
  username                   init  enter  dst    hp  ...    mIn   mAck   mRpt  ...  gaps  unprs
  script_user_1                 3      1    0    31  ...   ~190    >0     >0   ...     8    ~17
  script_user_2                 3      1    0    31  ...   ~190    >0     >0   ...     8    ~17
```
...
[script-clients] per-child event summary:
  username                   init  enter  dst    hp  ...    mIn   mAck   mRpt  ...  gaps  unprs
  script_user_1                 3      1    0    39  ...   ~190    >0     >0   ...     0    ~17
  script_user_2                 3      1    0    39  ...   ~190    >0     >0   ...     0    ~17
```

`hp=39 gaps=0` —— 对比场景 2 的 `hp=31 gaps=8`，这就是 RUDP reliable retransmit 工作的证据。
...
`samples/client/StressAvatar.cs` 和 `StressMovementBotComponent.cs` 的日志形态被
`src/tools/world_stress/client_event_tap.cc` 按字面量解析。
...
| `[StressAvatar:42] OnMovementCorrectionReportSent ack=A tick=T flags=F distance=D` | `mRpt` | owner replay 后调用 correction report 发送路径 |
