...
public static class Program
{
    public static int Main(string[] args)
    {
        ...
        Console.WriteLine($"CppEmitter: emitted {emitted}/{linked.Entities.Count} entity headers ({skipped} skipped) to {outputPath}");
        return 0;
    }
                                    ...
                                    ushort typeId)
    {
        ...
        // Structs first so their typed C++ types are in scope before the
        // entity body references them via container element / property type.
        EmitStructTypes(sb, entity, @namespace);
    ...
    private static void EmitStructTypes(StringBuilder sb, EntityDefModel entity, string @namespace)
    {
        // Forward-declare every struct so a struct field that references
        // another struct in the same entity compiles regardless of order.
        if (entity.Structs.Count > 0)
        {
            foreach (var s in entity.Structs) sb.AppendLine($"struct {s.Name};");
            sb.AppendLine();
        }
        foreach (var s in entity.Structs)
        {
            ...
            // Variant-side reader (used when struct lands as an entity property).
            ...
            sb.AppendLine("  }");
            // Wire integral writer (used as RPC arg or struct field).
            ...
            sb.AppendLine("  }");
            // Wire integral reader. Returns false on truncation so nested calls
            // propagate cleanly through DispatchRpc / outer Deserialize.
