...
internal static class ComponentIdManifestParser
{
    public const string FileName = "component_ids.xml";
    public const int kMinId = 1;
    public const int kMaxId = 0xFFFF;

    public static ComponentIdManifest? Parse(string xml, string sourcePath,
                                              Action<Diagnostic>? reportDiagnostic)
    {
        ...
        var root = doc.Root;
        if (root == null || root.Name.LocalName != "component_ids")
        {
            reportDiagnostic?.Invoke(Diagnostic.Create(
                DefDiagnosticDescriptors.DEF025, Location.None, sourcePath,
                "Root element must be <component_ids>"));
            return null;
        }
        ...
        foreach (var el in root.Elements("component"))
        {
            ...
            if (name == null || string.IsNullOrWhiteSpace(name))
            {
                reportDiagnostic?.Invoke(Diagnostic.Create(
                    DefDiagnosticDescriptors.DEF025, Location.None, sourcePath,
                    "<component> entry missing name attribute"));
                continue;
            }
        ...
        return new ComponentIdManifest(sourcePath, active, deprecatedIds, deprecatedNames);
    }
}
