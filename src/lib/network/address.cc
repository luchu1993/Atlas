...
auto Address::ToSockaddr() const -> sockaddr_in {
  sockaddr_in sa{};
...
auto Address::Resolve(std::string_view hostname, uint16_t port) -> Result<Address> {
  ...
  if (rc != 0 || result == nullptr) {
    return Error(ErrorCode::kNotFound, std::format("Failed to resolve hostname: {}", hostname_str));
  }
