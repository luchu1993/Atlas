#ifndef ATLAS_LIB_FOUNDATION_MEMORY_STREAM_BUFFER_POOL_H_
#define ATLAS_LIB_FOUNDATION_MEMORY_STREAM_BUFFER_POOL_H_
...
class StreamBufferPool {
 public:
  ...
  [[nodiscard]] auto Acquire(std::size_t min_size) -> StreamBuffer {
    auto tier = TierForSize(min_size);
    if (tier >= kNumTiers) {
 ...
 private:
  ...
  static auto TierForSize(std::size_t size) -> std::size_t {
    for (std::size_t i = 0; i < kNumTiers; ++i) {
      if (kSlabSizes[i] >= size) return i;
    }
    return kNumTiers;
  }
...
#endif  // ATLAS_LIB_FOUNDATION_MEMORY_STREAM_BUFFER_POOL_H_
