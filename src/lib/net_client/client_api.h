...
typedef void (*AtlasLoginResultFn)(void* user_data, uint8_t status,
                                   const char* baseapp_host,
                                   uint16_t baseapp_port,
                                   const char* error_message);

typedef void (*AtlasAuthResultFn)(void* user_data, uint8_t success,
                                  uint32_t entity_id, uint16_t type_id,
                                  const char* error_message);

// Legal only in DISCONNECTED. Result delivered from a future Poll.
ATLAS_NET_API int32_t AtlasNetLogin(AtlasNetContext* ctx,
...
ATLAS_NET_API int32_t AtlasNetAuthenticate(AtlasNetContext* ctx,
...
ATLAS_NET_API int32_t AtlasNetDisconnect(AtlasNetContext* ctx,
                                         AtlasDisconnectReason reason);

ATLAS_NET_API int32_t AtlasNetSendBaseRpc(AtlasNetContext* ctx,
                                          uint32_t entity_id, uint32_t rpc_id,
                                          const uint8_t* payload, int32_t len);

ATLAS_NET_API int32_t AtlasNetSendCellRpc(AtlasNetContext* ctx,
                                          uint32_t entity_id, uint32_t rpc_id,
                                          const uint8_t* payload, int32_t len);
...
typedef void (*AtlasPlayerBaseCreateFn)(AtlasNetContext* ctx,
                                        uint32_t entity_id, uint16_t type_id,
                                        const uint8_t* base_props,
                                        int32_t base_props_len);

typedef void (*AtlasPlayerCellCreateFn)(AtlasNetContext* ctx,
                                        uint32_t space_id,
                                        float pos_x, float pos_y, float pos_z,
                                        float dir_x, float dir_y, float dir_z,
                                        const uint8_t* cell_props,
                                        int32_t cell_props_len);

typedef void (*AtlasResetEntitiesFn)(AtlasNetContext* ctx);

typedef void (*AtlasEntityEnterFn)(AtlasNetContext* ctx,
                                   uint32_t entity_id, uint16_t type_id,
                                   float pos_x, float pos_y, float pos_z,
                                   float dir_x, float dir_y, float dir_z,
...
typedef void (*AtlasEntityPositionFn)(AtlasNetContext* ctx, uint32_t entity_id,
                                      float pos_x, float pos_y, float pos_z,
                                      float dir_x, float dir_y, float dir_z,
                                      uint8_t on_ground);

typedef void (*AtlasEntityPropertyFn)(AtlasNetContext* ctx, uint32_t entity_id,
                                      uint8_t scope,
                                      const uint8_t* delta, int32_t delta_len);

typedef void (*AtlasForcedPositionFn)(AtlasNetContext* ctx, uint32_t entity_id,
                                      float pos_x, float pos_y, float pos_z,
                                      float dir_x, float dir_y, float dir_z);

typedef void (*AtlasRpcCallbackFn)(AtlasNetContext* ctx, uint32_t entity_id,
                                   uint32_t rpc_id,
                                   const uint8_t* payload, int32_t len);
...
ATLAS_NET_API int32_t AtlasNetGetStats(AtlasNetContext* ctx,
