...
public:
    ...
    ~CancelRegistration()
    {
        if (state_ && id_ != 0)
        {
            ...
            cbs.erase(
                std::remove_if(cbs.begin(), cbs.end(), [id = id_](const CancellationState::Entry& e)
                               { return e.id == id; }),
                cbs.end());
        }
    }
        ...
        if (this != &other)
        {
            ...
            if (state_ && id_ != 0)
            {
                ...
                cbs.erase(std::remove_if(cbs.begin(), cbs.end(),
                                         [id = id_](const CancellationState::Entry& e)
                                         { return e.id == id; }),
                          cbs.end());
            }
...
public:
    ...
    [[nodiscard]] auto is_cancellation_requested() const -> bool { return state_->cancelled; }
...
public:
    ...
    [[nodiscard]] auto is_cancelled() const -> bool { return state_ && state_->cancelled; }
