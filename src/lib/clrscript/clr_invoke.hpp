...
public:
    ...
    [[nodiscard]] auto bind(ClrHost& host, const std::filesystem::path& assembly_path,
        ...
        -> Result<void>
    {
        auto result = host.get_method_as<FnPtr>(assembly_path, type_name, method_name);
        if (!result)
            return result.error();
        fn_ = *result;
        return {};
    ...
    {
        if (!fn_)
            return Error{ErrorCode::ScriptError, "ClrStaticMethod not bound"};
        ...
        {
            const int ret = fn_(args...);
            if (ret != 0 && has_clr_error())
