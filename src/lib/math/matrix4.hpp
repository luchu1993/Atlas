...
struct Matrix4
{
    ...
    [[nodiscard]] constexpr auto operator()(int row, int col) -> float& { return m[row * 4 + col]; }
    ...
    [[nodiscard]] static auto look_at(const Vector3& eye, const Vector3& target, const Vector3& up)
        -> Matrix4;
    ...
    [[nodiscard]] constexpr auto get_translation() const -> Vector3 { return {m[3], m[7], m[11]}; }
