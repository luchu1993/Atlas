...
auto Quaternion::FromEuler(float pitch, float yaw, float roll) -> Quaternion {
  float hp = pitch * 0.5f;
  float hy = yaw * 0.5f;
  float hr = roll * 0.5f;
...
auto Quaternion::FromMatrix(const Matrix4& mat) -> Quaternion {
  // Shepperd's method
...
auto Quaternion::ToMatrix() const -> Matrix4 {
