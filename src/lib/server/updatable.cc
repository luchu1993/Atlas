...
auto Updatables::Add(Updatable* object, int level) -> bool {
  ...
  if (object->removal_handle_ >= 0) return false;  // already registered
  ...
  for (int l = level + 1; l <= NumLevels(); ++l) ++level_offsets_[static_cast<std::size_t>(l)];

  object->removal_handle_ = insert_pos;

  // Fix up removal_handle_ for objects that shifted right.
  for (int i = insert_pos + 1; i < level_offsets_[static_cast<std::size_t>(NumLevels())]; ++i) {
...
void Updatables::Call() {
  ...
  int total = level_offsets_[static_cast<std::size_t>(NumLevels())];
...
void Updatables::Compact() {
  ...
  new_offsets[static_cast<std::size_t>(NumLevels())] = static_cast<int>(new_objects.size());
