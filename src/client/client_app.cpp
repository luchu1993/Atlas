...
auto ClientApp::init(int argc, char* argv[]) -> bool
{
    ...
    if (config_.password_hash.empty())
        config_.password_hash = "default_hash";

    // Start RUDP server (for receiving replies)
    auto listen_result = network_.start_rudp_server(Address("127.0.0.1", 0));
    if (!listen_result)
    {
        ATLAS_LOG_ERROR("Client: failed to start RUDP listener: {}",
                        listen_result.error().message());
        return false;
    }

    // Configure fast polling
    dispatcher_.set_max_poll_wait(std::chrono::milliseconds(1));
...
auto ClientApp::init_clr(const char* exe_path) -> bool
{
    ...
    if (!set_provider)
    {
        ATLAS_LOG_ERROR("Client: failed to resolve atlas_set_native_api_provider");
        return false;
    }
    (*set_provider)(native_provider_.get());

    // Resolve CLR error bridge
    auto error_set = native_api_library_->get_symbol<GetClrBridgeFn>("atlas_get_clr_error_set_fn");
    auto error_clear =
    ...
    auto load_result = script_engine_->load_module(config_.script_assembly);
    if (!load_result)
    {
...
void ClientApp::fini_clr()
{
    ...
    {
        auto set_provider = native_api_library_->get_symbol<SetNativeApiProviderFn>(
...
auto ClientApp::login() -> bool
{
    ...
    auto ch_result = network_.connect_rudp(login_addr);
    if (!ch_result)
    {
    ...
    (void)login_ch->send_message(req);

    // Wait for LoginResult
    std::optional<login::LoginResult> login_result;
    network_.interface_table().register_typed_handler<login::LoginResult>(
    ...
    if (!login_result || login_result->status != login::LoginStatus::Success)
    {
    ...
    ATLAS_LOG_INFO("Client: login succeeded, BaseApp at {}",
                   login_result->baseapp_addr.to_string());
...
{
    auto ch_result = network_.connect_rudp(baseapp_addr);
    if (!ch_result)
    {
    ...
    (void)baseapp_channel_->send_message(auth_msg);

    // Wait for AuthenticateResult
    std::optional<std::tuple<bool, EntityID, uint16_t>> auth_result;
    network_.interface_table().register_typed_handler<baseapp::AuthenticateResult>(
    ...
    ATLAS_LOG_INFO("Client: authenticated as entity={} type={}", player_entity_id_,
                   player_type_id_);
...
auto ClientApp::main_loop() -> int
{
    ...
    network_.interface_table().set_default_handler(
        [this](const Address&, Channel*, MessageID msg_id, BinaryReader& reader)
        {
            ...
            if (rem > 0)
            {
                auto payload = reader.read_bytes(rem);
                if (payload)
                {
